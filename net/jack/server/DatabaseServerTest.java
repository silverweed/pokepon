//: pokepon.net.jack/server/DatabaseServerTest.java
...
public class DatabaseServerTest extends MultiServerTest implements DatabaseServer {

	protected String dbName;
	protected URL dbURL;

	public DatabaseServerTest() throws IOException {
		this(ServerOptions.construct());
	}

	public DatabaseServerTest(ServerOptions opts) throws IOException {
		super(opts);
		loadOptions(opts);
		if(opts.database == null)
			setDatabaseLocation("file://"+Meta.getDataURL().getPath()+"/server.db"); 
		if(opts.serverName == null)
			serverName = DatabaseServerTest.class.getSimpleName();
		if(verbosity >= 0)
			printDebug("["+serverName+"] Constructed with database: "+dbURL+" (path: "+dbName+")");
	}

	@Override
	public DatabaseServerTest loadOptions(ServerOptions opts) {
		try {
			if(opts.database != null)
	...
	public void setDatabaseLocation(String db) throws MalformedURLException {
		...
		if(verbosity >= 1) printDebug("[DatabaseServerTest] Set database location to "+dbURL);
		if(!Files.exists(Paths.get(dbName))) {
			try {
				...
				if(verbosity >= 1) printDebug("[DatabaseServerTest] Created database file.");
			} catch(IOException e) {
				printDebug("[ ERROR ] Caught IOException while creating database: "+e);
			}
		} else if(verbosity >= 1) printDebug("[DatabaseServerTest] Database file already exists.");
	}

	public static void main(String[] args) {
		DatabaseServerTest server = null;

		try {
			server = new DatabaseServerTest();
			...
			printDebug("Caught IOException while starting DatabaseServerTest: ");
			server.shutdown();
			e.printStackTrace();
	...
	public boolean nickExists(String nick) throws FileNotFoundException {
		try (BufferedReader scanner = new BufferedReader(new InputStreamReader(new FileInputStream(dbName)))) {
			...
			throw new FileNotFoundException("Thrown by DatabaseServerTest.nickExists("+nick+")");
		} catch(Exception e) {
	...
	public List<String> getNicks() throws FileNotFoundException {
		...
		try (BufferedReader scanner = new BufferedReader(new InputStreamReader(new FileInputStream(dbName)))) {
			...
			throw new FileNotFoundException("Thrown by DatabaseServerTest.getNicks()");
		} catch(Exception e) {
	...
	public boolean checkPasswd(String nick,char[] passwd) throws FileNotFoundException {
		...
		try (BufferedReader scanner = new BufferedReader(new InputStreamReader(new FileInputStream(dbName)))) {
			...
			throw new FileNotFoundException("Thrown by: DatabaseServerTest.checkPasswd("+nick+",<passwd>)");
		} catch(IOException e) {
	...
	public int registerNick(String nick,char[] passwd) {
		...
		try {
			...
			printDebug("Database "+dbName+" not found. Creating it...");
			try {
	...
	public void loadConfiguration(ServerOptions opts) {
		...
		if(verbosity >= 1) printDebug("[DatabaseServerTest] loaded configuration.");
	}

	protected static void printUsage() {
		System.out.println("Usage: "+DatabaseServerTest.class.getSimpleName()+" [address (- for localhost)] [port] [verbosity]");
		System.exit(0);
	}
}
