//: pokepon.net.jack/server/MultiServerTest.java
...
/** A relatively complete implementation of a MultiThreadedServer with name validation;
 * it uses the Connection class, which can be modularly extended via ConnectionExecutors.
 *
 * @author silverweed
 */

public class MultiServerTest extends BasicNameValidatingServer implements MultiThreadedServer, AutoCloseable {
	...
	public MultiServerTest(ServerOptions opts) throws IOException {
		...
		if(opts.serverName == null)
			serverName = MultiServerTest.class.getSimpleName();
		if(verbosity >= 0)
			printDebug("["+serverName+"] Constructed with maxClients = "+maxClients);
	}

	@Override
	public MultiServerTest loadOptions(ServerOptions opts) {
	...
	public static void main(String[] args) {
		MultiServerTest server = null;

		try {
			server = new MultiServerTest();
			...
			printDebug("Caught IOException while starting MultiServerTest: ");
			server.shutdown();
			e.printStackTrace();
	...
	protected static void printUsage() {
		consoleMsg("Usage: "+MultiServerTest.class.getSimpleName()+" [port] [verbosity]");
		System.exit(0);
	}
}
