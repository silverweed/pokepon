		...
		switch(cmd) {
			case "btlreq": {
				...
				int choice = JOptionPane.showConfirmDialog(pClient,
					...
					" challenged you to a battle!",JOptionPane.YES_OPTION);	
					...
					public void run() {
						if(formats.size() == 1 && formats.toArray(new String[0])[0].startsWith("@")) {
							...
							if(pClient.showTeamChoiceDialog(formats.toArray(new String[0])[0].substring(1)))
								connection.sendMsg(CMN_PREFIX+"ok" + 
									(pClient.getFormat() != null 
										? " " + pClient.getFormat() 
										: ""));
							else
			...
			case "btlko":
				/* !btlko <id> [msg] */
				if(token.length < 2) return 1;
				pClient.append("Couldn't start battle #" + token[1] +
						(token.length > 2 ? ": "+ConcatenateArrays.merge(token,1) : "."));
